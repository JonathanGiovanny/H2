...
public class HServiceImpl implements HService {
  ...
  private H copyData(HDTO dto, H entity) {
    ...
    entity.setClicks(Utils.isNotNullOr(dto.getClicks(), entity.getClicks()));
    entity.setScore(Utils.isNotNullOr(dto.getScore(), entity.getScore()));

    HType ht = Optional.ofNullable(dto.getType()).filter(h -> h.getId().equals(entity.getId())).map(htService::toEntity)
